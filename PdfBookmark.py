...
def readBookmarkFromFile(bmPathName):
    ...
    for line in open(bmPathName):
        ...
        if not matchObj:
            print
            'bookmark file format error in: ' + line
            sys.exit(0)
...
class PdfBookmark(object):
    ...
    def exportBookmark(self, bookmarkFile):
        ...
        print
        "Export %s's bookmarks to %s finished!" % (self.pdfFileName, bookmarkFile)
    ...
    def importBookmark(self, bookmarkFile, saveAsPdfName=None):
        ...
        output.write(stream)
        print
        "Add bookmarks in %s to %s finished!" % (bookmarkFile, saveAsPdfName)
    ...
    def _addPageRatio(self, outlines, pageLabels):
        ...
        for i in range(0, len(outlines)):
            ...
            if idIndirect in pageLabels:
                ...
                print
                'Error: Page corresponds to IndirectObject %d not Found' % idIndirect
                sys.exit(-1)
            if '/Top' in outline:
                top = outline['/Top']
            else:
            ...
            print("top: "+str(top))
            print("zoom: "+str(zoom))
            print("pageHeight: "+str(pageHeight))
            outline['/Ratio'] = pageNum + (1 - float(top) / float(zoom) / float(pageHeight))
            outlines[i] = outline
